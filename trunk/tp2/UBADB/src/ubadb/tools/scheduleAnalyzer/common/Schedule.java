...
public abstract class Schedule
{
	...
	{
		//TODO: Casos de Test con BinaryLocking y TernaryLocking Mart�n Cammi
	 ...
	 * @param oneExecution mantiene la ejecuci�n parcial que se va obteniendo.
	 * @param grafo de precedencia
	 */
	 ...
	 * Precondici�n: el grafo no debe ser serializable.
	...
	private void findCycle(List<String> cycle, List<Par<String,Boolean>> nodos, ScheduleGraph graph, boolean shutdownAlgorithm){
		...
		//Todos los nodos est�n deshabilitados y no se pide shutdown.
		if(!isEmpty(nodos) && !shutdownAlgorithm){
			for (int i = 0; i < nodos.size(); i++)
			{
				String tnx = (String) nodos.get(i).getFirst();
				
				//Si la tnx est� habilitada.
				if(nodos.get(i).getSecond())
				{
	...
	private boolean isEmpty(List<Par<String,Boolean>> lista){
		...
		return !algunoEsTrue; //Si ninguno es true -> est� vacia.
	}
	...
	private boolean tieneCiclosA(List<List<String>> possibleExcecutions, List<String> cycle)
	{
		...
		return nodos.size() > 0; //Si qued� algun nodo en la lista significa que algunos de ellos o todos forman un ciclo
	}*/
